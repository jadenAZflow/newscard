...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    switch (action) {
      case 'analyzeNewsLink':
        return res.json(await analyzeNewsLink(payload.url));
      case 'analyzeNewsContent':
        return res.json(await analyzeNewsContent(payload.content));
      case 'analyzeStyle':
        return res.json(await analyzeStyle(payload.base64Image));
      case 'generateCardBackground':
        return res.json(await generateCardBackground(payload.news, payload.style));
      default:
...
async function analyzeNewsLink(url: string) {
...
async function analyzeNewsContent(content: string) {
  const response = await ai.models.generateContent({
    model: 'gemini-2.5-flash',
...
async function generateCardBackground(news: any, style: any) {
